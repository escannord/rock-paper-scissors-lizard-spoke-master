...
async function picked(choice, elt) {
    ...
    var clone = $(".option").clone().removeClass("option").addClass("selected");
    ...
    await setTimeout(() => {
        ...
        var old_score = localStorage.getItem("paper_scissors_rock_score") ?? 0;
        if (house_choice == choice) {
            $(".result-text").text("EQUALITY");
            $(".btn-replay").toggleClass("btn-close btn-win");
        } else if (rules[house_choice].includes(choice)) {
